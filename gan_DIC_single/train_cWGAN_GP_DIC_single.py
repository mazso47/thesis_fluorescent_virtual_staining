...
def replace_blank_channels(fake_imgs, real_imgs):
    for i in range(fake_imgs.size(0)):                          # batch
        for channel in range(fake_imgs.size(1)):                # channels
            if torch.all(real_imgs[i, channel, :, :] == 0):     # check if target is blank
                fake_imgs[i, channel, :, :] = 0 
    return fake_imgs
